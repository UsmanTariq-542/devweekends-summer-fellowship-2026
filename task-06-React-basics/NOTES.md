...
- Functional state updates (`setCount(prev => prev + 1)`) vs direct value (`setCount(count + 1)`) — functional form needed only when new state depends on rapid/multiple updates in the same handler
- React StrictMode double-invokes state updater functions in dev mode — side effects (like logging) should never live inside a setState updater, only pure calculations should
